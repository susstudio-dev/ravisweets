...
const FRAMES: Frame[] = [
  ...
  },
  {
    ...
    body: 'Saffron-gold pearls dropped through a brass jhara into hot ghee — a single ladleful, never a vat. The Boondi Laddu we tie up in the afternoon starts as 200,000 of these.',
  ...
  },
  {
    ...
    body: 'From the Khammam counter or the Kondapur branch, every hamper leaves with a date stamp and a paisley tag — sealed by hand, two pairs of eyes before the lid closes.',
        ...
        <div className="flex snap-x snap-mandatory overflow-x-auto">
          {FRAMES.map((f) => (
            <article key={f.key} className="flex w-[88vw] shrink-0 snap-center flex-col gap-3 px-4">
              <div
                className="shadow-soft relative aspect-[4/5] overflow-hidden rounded-2xl p-6 ring-1 ring-[color:var(--color-border)]"
              ...
              <p className="text-theme-ink/75 text-sm leading-relaxed">{f.body}</p>
              <a
                href={f.shopHref}
        ...
        >
            ...
            <div className="container-site mt-10 grid grid-cols-1 gap-8 pb-16">
              {FRAMES.map((f) => (
                <article key={f.key} className="grid gap-4 md:grid-cols-[1fr_1.2fr]">
                  <div
                    className="shadow-soft relative aspect-[4/5] overflow-hidden rounded-2xl p-8 ring-1 ring-[color:var(--color-border)]"
                    style={{ background: frameBackdrop(f) }}
                  >
                    <Image
                      src={f.image}
                  ...
                  <div className="flex flex-col justify-center gap-3">
                    ...
                    <p className="text-theme-ink/75 leading-relaxed">{f.body}</p>
                  </div>
                </article>
              ))}
            </div>
            ...
            <div className="relative mt-auto flex-1">
              ...
              >
                {FRAMES.map((f, i) => (
                  ...
                  >
                    <div
                      className="shadow-lifted relative aspect-[4/5] w-[32vw] max-w-[480px] shrink-0 overflow-hidden rounded-[1.75rem] p-10 ring-1 ring-[color:var(--color-border)]"
                      style={{ background: frameBackdrop(f) }}
                    >
                      <Image
                        src={f.image}
                      ...
                      <span
                        className="font-display text-theme-ink/[0.08] pointer-events-none absolute -bottom-3 -right-1 select-none text-[14rem] leading-none"
                        aria-hidden="true"
                      >
                    ...
                    <div className="flex max-w-xl flex-col gap-4">
                      <p className="text-theme-accent flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.22em]">
                        <span
                      ...
                      <p className="text-theme-ink/75 text-lg leading-relaxed">{f.body}</p>
                      <a
                        href={f.shopHref}
                        className="bg-theme-ink shadow-soft hover:shadow-lifted group mt-2 inline-flex w-fit items-center gap-2 rounded-full px-5 py-2.5 text-sm font-semibold text-[color:var(--theme-base)] transition-all duration-300 hover:-translate-y-0.5"
                      >
                        {f.shopLabel}
          ...
          {!reduced && (
            <div className="container-site pb-10">
              <div className="bg-theme-ink/10 relative h-px w-full">
                <motion.div
